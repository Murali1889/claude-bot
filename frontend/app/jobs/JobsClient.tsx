...
function JobCard({ job, isSelected, isUpdated, onSelect, getStageIndex }: any) {
  ...
  // For in-progress jobs: Simple layout without progress tracker
  return (
    <div className={`bg-white dark:bg-gray-800 rounded-lg border transition-all duration-300 ${
      isUpdated ? "border-indigo-400 shadow-lg shadow-indigo-100 dark:shadow-indigo-900/20" : "border-gray-200 dark:border-gray-700 hover:shadow-md"
    }`}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-2">
              ...
              <div className={`px-2 py-0.5 rounded text-xs font-medium ${
                job.status === 'pending' ? 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400' :
                job.status.includes('rca') ? 'bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400' :
                job.status.includes('code') ? 'bg-purple-100 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400' :
                job.status.includes('pr') ? 'bg-indigo-100 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-400' :
                'bg-yellow-100 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-400'
              }`}>
                {STATUS_STAGES.find(s => s.key === job.status)?.label || job.status}
              </div>
            </div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-white">{job.problem_statement}</h3>
          </div>
        </div>
